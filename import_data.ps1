...
$collections | % {
    $collection = $_
    $importCommand = "docker exec -it $mongoDbContainer /bin/bash -c 'mongoimport --uri mongodb://mongo:Park123!@localhost:27017/seait --authenticationDatabase=admin --collection $collection --type json --file /tmp/data/$($database)db-$($collection).json'"
